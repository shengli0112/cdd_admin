<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="开屏图跳转链接" width="140">
        <template scope="scope">
      ...
      <el-table-column label="显示图片" width="120">
        <template scope="scope">
          <img :src="scope.row.screenUrl"  min-width="70" height="90" v-if="scope.row.screenUrl.length>0" :data-img="scope.row.screenUrl" type="text" size="small" @click="$imgPreview" />
    ...
    methods: {
      ...
      handleRecover(row) {
        MessageBox.confirm('您确定恢复该轮播图么', '确定', {
          ...
          recoverOpenScreen(row.id).then(response => {
            // this.flag = response.data.flag;
      ...
      handleDelete(row) {
        MessageBox.confirm('您确定删除该轮播图么', '确定删除', {
          ...
          deleteOpenScreen(row.id).then(response => {
            // this.flag = response.data.flag;
            if(response.data.flag == 1){
              this.$notify({
                title: '成功',
      ...
      create() {
          this.temp.screenUrl = this.uploadPicUrl
          createOpenScreen(this.temp).then(response => {
              if (response.data.flag === 1) {
                  this.$notify({
                      title: '成功',
      ...
      update() {
          this.temp.screenUrl = this.uploadPicUrl
          updateOpenScreen(this.temp).then(response => {
              if (response.data.flag === 1) {
                  this.$notify({
                      title: '成功',
                      message: response.data.message,
