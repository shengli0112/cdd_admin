...
<!--          <span class="link-type" @click='handleFetchPv(scope.row.pageviews)'>{{scope.row.pageviews}}</span>-->
      ...
      <el-table-column align="center" label="操作" width="170">
        <template scope="scope">
          ...
          <span class="link-type" @click="handleDelete(scope.row)">删除</span>
          <span class="link-type" @click="handleUpdate(scope.row)">置顶</span>
          <span v-if="scope.row.status=='0'" class="link-type" @click="handleUpdate(scope.row)">恢复</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import { fetchHouseList,deleteHouse } from 'api/house_table';
