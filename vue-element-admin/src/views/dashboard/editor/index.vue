<template>
  <div class="dashboard-editor-container">

    <panel-group @handleSetLineChartData="handleSetLineChartData" />

    <el-row style="background:#fff;padding:16px 16px 0;margin-bottom:32px;">
      <line-chart :chart-data="lineChartData" />
    </el-row>

    <el-row>
      <el-col :span="6">
        <el-card class="box-card">
          <div slot="header" class="box-card-header">
            <pan-thumb class="panThumb" :image="avatar"> 你的权限:
              <span class="pan-info-roles" :key='item' v-for="item in roles">{{item}}</span>
            </pan-thumb>
          </div>
          <span class="display_name">{{name}}</span>
          <div class="info-item">
            <countTo class="info-item-num" :startVal='0' :endVal='statisticsData.cfCount'
                     :duration='3400'></countTo>
            <span class="info-item-text">厂房</span>
            <icon-svg icon-class="a" class="dashboard-editor-icon"></icon-svg>
          </div>
          <div class="info-item">
            <countTo class="info-item-num" :startVal='0' :endVal='statisticsData.tdCount'
                     :duration='3400'></countTo>
            <span class="info-item-text">土地</span>
            <icon-svg icon-class="a" class="dashboard-editor-icon"></icon-svg>
          </div>
          <div class="info-item">
            <countTo class="info-item-num" :startVal='0' :endVal='statisticsData.ckCount'
                     :duration='3600'></countTo>
            <span class="info-item-text">仓库</span>
            <icon-svg icon-class="b" class="dashboard-editor-icon"></icon-svg>
          </div>
        </el-card>
      </el-col>

      <el-col :span="8">
        <pie-chart></pie-chart>
      </el-col>

      <el-col :span="10">
        <bar-chart></bar-chart>
      </el-col>
    </el-row>

<!--    <el-row :gutter="20">-->
<!--      <el-col :span="15">-->
<!--        <line-chart></line-chart>-->
<!--      </el-col>-->
<!--      <el-col :span="9">-->
<!--        <todo-list></todo-list>-->
<!--      </el-col>-->
<!--    </el-row>-->

  </div>
</template>

<script>
  import { mapGetters } from 'vuex';
  import panThumb from 'components/PanThumb';
  import pieChart from './pieChart';
  import barChart from './barChart';
  import lineChart from './lineChart';
  import panelGroup from './panelGroup';
  import countTo from 'vue-count-to';
  import todoList from 'components/TodoList';
  import {
    fetchHouseCount,
    fetchStatistics
  } from 'api/home';

  // var lineChartData = null

  export default {
    name: 'dashboard-editor',
    components: { panThumb, countTo, pieChart, lineChart, barChart, todoList, panelGroup },
    data() {
      return {
        statisticsData: null,
        lineChartData: null
      }
    },
    created() {
      this.getHouseCount();
      this.getStatistics()
    },
    methods: {
      getHouseCount() {
        fetchHouseCount().then(response => {
          this.statisticsData = response.data.data;
        })
      },
      getStatistics() {
        fetchStatistics().then(response => {
          this.lineChartData = response.data.data;
        })
      },
      handleSetLineChartData(type) {
          this.lineChartData = this.lineChartData[type]
      }
    },
    computed: {
      ...mapGetters([
        'name',
        'avatar',
        'roles'
      ])
    }
  }
</script>

<style rel="stylesheet/scss" lang="scss" scoped>
  .dashboard-editor-container {
    margin: 30px;

    .btn-group {
      margin-bottom: 60px;
    }

    .box-card-header {
      position: relative;
      height: 160px;
    }

    .panThumb {
      z-index: 100;
      height: 150px;
      width: 150px;
      position: absolute;
      left: 0px;
      right: 0px;
      margin: auto;
    }

    .display_name {
      font-size: 30px;
      display: block;
    }

    .info-item {
      display: inline-block;
      margin-top: 10px;
      font-size: 14px;

      &:last-of-type {
        margin-left: 15px;
      }
    }
  }
</style>
