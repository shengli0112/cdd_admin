  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" label="租售" width="65">
        <template scope="scope">
          <span>{{scope.row.entrustUseType}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="类型" width="65">
        <template scope="scope">
          <span>{{scope.row.entrustType}}</span>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        fetchEntrustList(this.listQuery).then(response => {
      ...
      handleDelete(row) {
        MessageBox.confirm('您确定删除该委托么', '确定删除', {
          confirmButtonText: '确定删除',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          deleteEntrust(row.entrustId).then(response => {
            // this.flag = response.data.flag;
            if (response.data.flag === 1) {
              this.$notify({
                title: '成功',
