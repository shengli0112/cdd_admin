<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="链接描述" width="125">
        <template scope="scope">
          <span>{{scope.row.linkDesc}}</span>
        </template>
      </el-table-column>

        <el-table-column align="center" label="链接地址" width="130">
          <template scope="scope">
            <span>{{scope.row.linkUrl}}</span>
          </template>
        </el-table-column>

        <el-table-column label="列表图片" width="120">
          <template scope="scope">
            <img :src="scope.row.homeImage"  min-width="70" height="70" />
          </template>
        </el-table-column>
...
<script>
  ...
  import { fetchNewsList,recoverNews,deleteNews } from 'api/news';
  ...
  export default {
    ...
    methods: {
      ...
      handleRecover(row) {
        MessageBox.confirm('您确定恢复该新闻吗？', '确定', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          recoverNews(row.id).then(response => {
      ...
      handleDelete(row) {
          MessageBox.confirm('您确定删除该新闻吗？', '确定', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
          }).then(() => {
